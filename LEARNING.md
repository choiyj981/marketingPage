...
**현재 상황:**
- ✅ Cloud Run 인스턴스 1개 (웹사이트 서버) - 생성 완료
- ❌ Cloud SQL 인스턴스 1개 (데이터베이스 서버) - 생성 필요
- **총 2개의 인스턴스 필요**

**비유:**
- **식당 비유:**
  - 컴퓨터 1번 (Cloud Run) = 식당의 홀 (손님 접대)
  - 컴퓨터 2번 (Cloud SQL) = 식당의 냉장고 (재료 저장)

**인스턴스 확장:**
- **인스턴스 1개**: 사용자 100명 처리 가능
- **인스턴스 2개**: 사용자 200명 처리 가능 (부하 분산)
- 트래픽에 따라 자동으로 인스턴스 증가 가능
...
### 진행 중인 작업
- ⏳ Cloud SQL 인스턴스 생성 (데이터베이스 서버)
- ⏳ Cloud Run과 Cloud SQL 연결
- ⏳ 환경 변수 설정
- ⏳ 데이터베이스 스키마 생성
...
- [ ] 데이터베이스 생성
- [ ] Cloud Run 연결
- [ ] 환경 변수 설정 (DATABASE_URL, SESSION_SECRET)
- [ ] 데이터베이스 스키마 생성
- [ ] 웹사이트 접속 테스트
- [ ] 데이터베이스 연결 확인
- [ ] 사용자 인증 기능 테스트
...
**Cloud SQL 관련:**
```bash
# Cloud SQL 인스턴스 목록 확인
gcloud sql instances list

# Cloud SQL 인스턴스 연결 정보 확인
gcloud sql instances describe [인스턴스ID]
```

**참고 문서:**
- `GCP_CLOUD_SQL_SETUP.md` - Cloud SQL 설정 가이드 (비유 중심)
...
### 2025-11-05 (Google Cloud Run 배포)
...
### 2025-11-05 (인스턴스 개념 학습)
- 인스턴스 개념 이해 (컴퓨터 = 인스턴스)
- Cloud Run 인스턴스 1개 생성 완료 (웹사이트 서버)
- Cloud SQL 인스턴스 생성 필요 (데이터베이스 서버)
- GCP_CLOUD_SQL_SETUP.md 가이드 문서 생성 (비유 중심)
